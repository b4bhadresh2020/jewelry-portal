    ...
    <ul class="sidenav sidenav-collapsible leftside-navigation collapsible sidenav-fixed menu-shadow" id="slide-out"
        ...
        @if(!empty($menuData[0]) && isset($menuData[0]))
            @foreach ($menuData[0]->menu as $menu)
                @if(isset($menu->navheader) || isset($menu->all_access) || (isset($menu->permissions) && auth()->user()->hasAnyHavePermission(@$menu->permissions) || isDeveloper())) {{-- check Permission --}}
                    ...
                    @else
                        ...
                        <li class="bold {{(request()->is($menu->url)) ? 'active' : '' }}">
                            ...
                            @if(isset($menu->submenu))
                                @include('panels.submenu', ['menu' => $menu->submenu, 'peta'=>false])
                            @endif
                        </li>
                    @endif
                @endif
