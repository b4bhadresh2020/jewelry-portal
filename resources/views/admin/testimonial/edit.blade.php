                ...
                <div class="col s6 right-align">
                    @permission('view-testimonial')
                        <a href="{{ url('admin/testimonial') }}"
                            ...
                            <i class="material-icons left">menu</i> View All Testimonial
                        </a>
                    @endpermission
                </div>
            </div>
            <form method="POST" action="{{ url('admin/testimonial') }}/{{ $testimonial->id }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="row mt-4">
                    <div class="col s12">
                    ...
                    @foreach ($findActiveLanguage as $item)
                        <div id="{{$item->name}}" class="col mt-2 s12">
                            <div class="input-field col m6 s12">
                                <input id="name{{$item->id}}" type="text" value="{{@$testimonial->translate(@$item->code)->name}}"
                                    name="name:{{$item->code}}">
                            ...
                            <div class="input-field col m6 s12">
                                <input id="role{{$item->id}}" type="text" value="{{@$testimonial->translate(@$item->code)->role}}"
                                    name="role:{{$item->code}}">
                                <label for="role{{$item->id}}">Name ({{$item->name}})</label>
                                <small class="errorTxt2">
                            ...
                            <div class="input-field col m12 s12">
                                ...
                                <textarea  class="summernote mt-3" name="description:{{$item->code}}"
                                    value="{{ old('description:'.$item->code) }}" >{{@$testimonial->translate(@$item->code)->description}}</textarea>
                                <small class="errorTxt2">
